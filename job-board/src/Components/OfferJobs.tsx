...
function Offers({ isGrid, handleDeleteJob }: SavedProps) {
    ...
    <section>
        ...
        <div className="your-jobs-title">
            <div className="heading">
                <FaNewspaper />
                Your Applied Jobs: {offers.length}
            </div>
        </div>
                ...
                offers.map((job: JobType, index: number) => (
                    <div className={isGrid ? "appliedJob-grid" : "appiedJob"} key={index}>
                        <div className="dashboard-card">
                            ...
                            <button onClick={() => handleDeleteJob(index)} style={{marginTop: '8px'}}>Delete</button>
                        </div>
                    </div>
                ))
            )}
